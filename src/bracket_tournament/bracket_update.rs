...
pub async fn update_bracket(ctx: &Context<'_>) -> Result<(), Error> {
    ...
    for round_number in 1..=config.get("total").unwrap().as_i32().unwrap() {
        ...
        while let Some(current_document) = database.try_next().await? {
            ...
            rounds.push((
                ...
                (find_enemy_by_match_id_and_self_tag(ctx, &region, &round_number, &match_id, tag)
                    .await)
                    .unwrap()
                    .get("tag")
                    .unwrap()
    ...
    let output = Command::new("python")
        ...
        .arg(
            rounds
                .iter()
                .map(|(round, player1_tag, player2_tag)| {
                    format!("{}|{}|{}", round, player1_tag, player2_tag)
                })
                .collect::<Vec<String>>()
                .join(","),
        )
    ...
    match poise::serenity_prelude::ChannelId(bracket_channel_data.0.unwrap())
        .edit_message(&ctx, bracket_channel_data.1.unwrap(), |m| {
