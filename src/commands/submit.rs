...
pub async fn submit(ctx: Context<'_>) -> Result<(), Error> {
    ...
    for log in logs.clone() {
        ...
        let player1_log = log["battle"]["teams"][0][0]["tag"]
            .to_string()
            .strip_quote();
        let player2_log = log["battle"]["teams"][1][0]["tag"]
            .to_string()
            .strip_quote();
        if mode == *MODE
        ...
        if is_victory {
            ctx.send(|s| {
                s.reply(true).ephemeral(false).embed(|e| {
                    e.title("The result is in!").description(format!(
                        ...
                        caller["name"].to_string().strip_quote(),
                        caller["tag"].to_string().strip_quote()
                    ))
                })
            })
            .await?;
            ...
            ctx.send(|s| {
                s.reply(true).ephemeral(false).embed(|e| {
                    e.title("The result is in!").description(format!(
                        "Congratulations! {}({}) is ascended to next round!",
                        enemy["name"].to_string().strip_quote(),
                        enemy["tag"].to_string().strip_quote()
                    ))
                })
            })
            .await?;
